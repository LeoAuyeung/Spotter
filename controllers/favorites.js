...
async function getUserFavorites(req, res, next) {
	try {
		...
		const userFavorites = await database.favorites.findAll(({ raw: true , where: { 
			[Op.or]: [
				{ userId_1: currentUser.id,  },
...
async function createFavorite(req, res, next) {
	try {
		...
		let newFavorite = { "id": String(latestInt + 1) };
...
async function deleteFavorite(req, res, next) {
	try {
        ...
        const currentFavorite = await database.favorites.findOne({ raw: true , where: { id: id } });
        if (currentFavorite == null) {
            return res.status(401).json({ code: "error", message: "Favorite does not exist." });
        }
        
        // Delete the Favorite
        const deleted = await database.favorites.destroy({ where: { id: id } });
        if (deleted) {
            return res.status(201).json({ code: "success", message: "Favorite successfully deleted." });
        }
    }
    catch (err) {
        console.log(err);
        return res.status(500).json({ code: "error", message: "Error with deleting Favorite. Please retry." });
	}
}
