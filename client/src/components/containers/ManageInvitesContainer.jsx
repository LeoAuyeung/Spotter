...
class ManageInvitesContainer extends Component {
	...
	async componentDidMount() {
		...
		let currentPotentialInvites = await axios.get(
			`${BASE_URL}/api/users/pendinginvites`,
			{ headers }
		);
	...
	async acceptInvite(inviteId) {
		...
		let currentPotentialInvites = await axios.post(
	...
	async denyInvite(inviteId) {
		...
		var data = {};
		let currentPotentialInvites = await axios.post(
			`${BASE_URL}/api/sessions/${sessionId}/deny`,
			data,
			{ headers: { authorization: localStorage.token } }
		);
		window.location.reload();
	...
	render() {
		if (this.state.currInvites == undefined) {
			return "";
		} else if (this.state.currInvites.length === 0) {
			return (
				<div>
					<h1>Invites</h1>
		...
		var newList = this.state.currInvites.map((currInvite) => {
			return (
				<div>
					...
					<button
						type="button"
						onClick={() => this.acceptInvite(currInvite.session.id)}
					>
						Accept
					</button>
					<button
						type="button"
						onClick={() => this.denyInvite(currInvite.session.id)}
					>
						Deny
					</button>
					<br></br>
