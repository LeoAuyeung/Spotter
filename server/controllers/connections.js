...
async function getUserConnections(req, res, next) {
	try {
		...
		const userConnections = await database.connections.findAll(({ raw: true , where: { 
			[Op.or]: [
...
async function deleteConnection(req, res, next) {
	try {
        ...
        const currentConnection = await database.connections.findOne({ raw: true , where: { int: id } });
        if (currentConnection == null) {
            return res.status(401).json({ code: "error", message: "Connection does not exist." });
        }
        
        // Delete the Connection
        const deleted = await database.connections.destroy({ where: { int: id } });
        if (deleted) {
            return res.status(201).json({ code: "success", message: "Connection successfully deleted." });
        }
    }
    catch (err) {
        console.log(err);
        return res.status(500).json({ code: "error", message: "Error with deleting Connection. Please retry." });
	}
}
